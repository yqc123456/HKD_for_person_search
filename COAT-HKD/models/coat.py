...
from models.teacher_head import COAT_REID
from loss.kd_loss import KLLoss, KD_feat_loss, KD_sim_kl_loss
...
class COAT(nn.Module):
    def __init__(self, cfg):
        ...
        self.lw_kdlogits = cfg.lw_kdlogits
        self.lw_kdfeat = cfg.lw_kdfeat
        self.lw_oim_loss2 = cfg.lw_oim_loss2
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        _, rcnn_losses, feat_det2, label_det2, feat_det3, label_det3, feat_ins2, label_ins2, feat_ins3, label_ins3 = self.roi_heads(
            features,
            boxes,
            images.image_sizes,
            targets)
        ...
        return losses, feat_det2, label_det2, feat_det3, label_det3, feat_ins2, label_ins2, feat_ins3, label_ins3
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.kd_logits_loss = KLLoss(2, cfg.kdlogits_bidir)
        self.kd_feat_loss = KD_feat_loss
        self.teacher_head2 = COAT_REID(cfg)
        self.teacher_head3 = deepcopy(self.teacher_head2)
        self.show_logit_precision_cnt = 0
        self.tempsimkl = cfg.tempsimkl
        self.simkl_bidir = cfg.simkl_bidir
        # rename the method inherited from parent class
        self.postprocess_proposals = self.postprocess_detections
        self.lw_kdlogits = cfg.lw_kdlogits
        self.lw_kdfeat = cfg.lw_kdfeat
        self.lw_oim_loss2 = cfg.lw_oim_loss2
        self.sim_use_KL = cfg.sim_use_KL
        self.lw_kd_sim_kl = cfg.lw_kd_sim_kl
        self.kd_sim_kl = KD_sim_kl_loss
        self.use_reid_head2 = cfg.use_reid_head2
        self.use_reid_head3 = cfg.use_reid_head3
        # evaluation        
        self.eval_feat = cfg.EVAL_FEATURE
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        Arguments:
            ...
            targets (List[Dict])
        """
        self.show_logit_precision_cnt += 1
        cws = True
        gt_det_2nd = None
        gt_det_3rd = None
        feat_det2 = None
        ...
        label_det2 = None
        label_det3 = None
        feat_ins2, label_ins2, feat_ins3, label_ins3 = None, None, None, None
        ...
        teacher_input_2nd = box_features.clone().detach()
        ...
        # --------------------- The third stage -------------------- #
        box_features = self.box_roi_pool(features, boxes, image_shapes)
        teacher_input_3rd = box_features.clone().detach()
        if not self.training:
        ...
        if self.training:
            ...
            loss_rcnn_reid_2nd, feat_det2, label_det2, logits_det2 = self.reid_loss_2nd(box_embeddings_2nd,
                                                                                        box_pid_labels_2nd)
            loss_rcnn_reid_3rd, feat_det3, label_det3, logits_det3 = self.reid_loss_3rd(box_embeddings_3rd,
                                                                                        box_pid_labels_3rd)
            losses.update(loss_rcnn_reid_2nd=loss_rcnn_reid_2nd)
            losses.update(loss_rcnn_reid_3rd=loss_rcnn_reid_3rd)
            if self.use_reid_head2:
                oim_loss2, feat_ins2, label_ins2, logits_ins2 = self.teacher_head2(teacher_input_2nd,
                                                                                   box_pid_labels_2nd)
                losses.update(oim_loss2=oim_loss2 * self.lw_oim_loss2)

            if self.use_reid_head3:
                oim_loss3, feat_ins3, label_ins3, logits_ins3 = self.teacher_head3(teacher_input_3rd,
                                                                                   box_pid_labels_3rd)
                losses.update(oim_loss3=oim_loss3 * self.lw_oim_loss2)

            if self.lw_kdfeat > 0:
                if self.use_reid_head2:
                    kd_feat2 = self.kd_feat_loss(feat_ins2, feat_det2) * self.lw_kdfeat
                    losses.update(kd_feat2=kd_feat2)
                if self.use_reid_head3:
                    kd_feat3 = self.kd_feat_loss(feat_ins3, feat_det3) * self.lw_kdfeat
                    losses.update(kd_feat3=kd_feat3)
            if self.lw_kdlogits > 0:
                if self.use_reid_head2:
                    kd_logits2 = self.kd_logits_loss(logits_det2, logits_ins2) * self.lw_kdlogits
                    losses.update(kd_logits2=kd_logits2)
                if self.use_reid_head3:
                    kd_logits3 = self.kd_logits_loss(logits_det3, logits_ins3) * self.lw_kdlogits
                    losses.update(kd_logits3=kd_logits3)
            if self.lw_kd_sim_kl > 0:
                if self.use_reid_head2:
                    kd_sim_kl2 = self.kd_sim_kl(feat_ins2, feat_det2, self.sim_use_KL, self.tempsimkl, self.simkl_bidir)
                    losses.update(kd_sim_kl2=kd_sim_kl2 * self.lw_kd_sim_kl)
                if self.use_reid_head3:
                    kd_sim_kl3 = self.kd_sim_kl(feat_ins3, feat_det3, self.sim_use_KL, self.tempsimkl, self.simkl_bidir)
                    losses.update(kd_sim_kl3=kd_sim_kl3 * self.lw_kd_sim_kl)
            if self.show_logit_precision_cnt % 40 == 0:
                if self.use_reid_head2:
                    compare_logits(logits_det2, logits_ins2, label_det2, label_ins2, 'oim2')
                if self.use_reid_head3:
                    compare_logits(logits_det3, logits_ins3, label_det3, label_ins3, 'oim3')
        else:
            if self.eval_feat == 'stage2':
        ...
        return result, losses, feat_det2, label_det2, feat_det3, label_det3, feat_ins2, label_ins2, feat_ins3, label_ins3

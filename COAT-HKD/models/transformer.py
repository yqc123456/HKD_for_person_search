...
from utils.mask import exchange_token, exchange_patch, get_mask_box, jigsaw_token, cutout_patch, erase_patch, \
...
class TransformerHead(nn.Module):
    ...
    def forward(self, box_features):
        # 要交换的是token bank的每一层，所以mask box对应到feature中的区域就是贯穿某几行，行数由mask size决定
...
class Transformers(nn.Module):
    def __init__(
        ...
        # 对每个尺度，都生成了一个 卷积，交换，transformer 的流程
    ...
    def forward(self, inputs):
        ...
        if self.training and self.use_feature_mask and self.feature_aug_type == 'exchange_patch':
            ...
            # 对特征条状进行mask，正常的那种mask
            enc_feat = feature_mask(enc_feat)

        # 对feature切分，对每个部分执行一次transformer组合流程

...
def main(args):
    ...
    softmax_criterion_s2_teacher = None
    softmax_criterion_s3_teacher = None
    if cfg.MODEL.LOSS.USE_SOFTMAX:
        softmax_criterion_s2 = SoftmaxLoss(cfg)
        softmax_criterion_s3 = SoftmaxLoss(cfg)
        if cfg.lw_share_celoss:
            softmax_criterion_s2_teacher = softmax_criterion_s2 if cfg.use_reid_head2 else None
            softmax_criterion_s3_teacher = softmax_criterion_s3 if cfg.use_reid_head3 else None
        else:
            softmax_criterion_s2_teacher = SoftmaxLoss(cfg) if cfg.use_reid_head2 else None
            softmax_criterion_s3_teacher = SoftmaxLoss(cfg) if cfg.use_reid_head3 else None
        softmax_criterion_s2.to(device)
        softmax_criterion_s3.to(device)
        if cfg.use_reid_head2:
            softmax_criterion_s2_teacher.to(device)
        if cfg.use_reid_head3:
            softmax_criterion_s3_teacher.to(device)
    ...
    if cfg.MODEL.LOSS.USE_SOFTMAX:
        ...
        if cfg.use_reid_head2:
            params_softmax_s2_teacher = [p for p in softmax_criterion_s2_teacher.parameters() if p.requires_grad]
            params.extend(params_softmax_s2_teacher)
        if cfg.use_reid_head3:
            params_softmax_s3_teacher = [p for p in softmax_criterion_s3_teacher.parameters() if p.requires_grad]
            params.extend(params_softmax_s3_teacher)
    ...
    if args.reid_ckpt is not None:
        resume_from_ckpt(args.reid_ckpt, model.roi_heads.teacher_head2)
        resume_from_ckpt(args.reid_ckpt, model.roi_heads.teacher_head3)
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        train_one_epoch(cfg, model, optimizer, train_loader, device, epoch, tfboard, softmax_criterion_s2,

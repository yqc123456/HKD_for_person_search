...
def KD_feat_loss(feat_reid, feat_srch):
    ...
    # loss=sum(  1-feat_reid[i].unsqueeze(dim=0) @ feat_srch[i].unsqueeze(dim=1)  \
    # for i in range(len(feat_srch)))/ len(feat_srch)
    # print('  kd loss', loss)
    return loss


def KD_sim_kl_loss(feat_gt, feat_det, temp=4, bidir=True):

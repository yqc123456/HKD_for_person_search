...
class OIMLoss(nn.Module):
    ...
    def forward(self, inputs, roi_label):
        ...
        inds = label >= 0
        pid_inds = (label >= 0) & (label != 5554)
        pid_label = label[pid_inds]
        pid_inputs = inputs[pid_inds.unsqueeze(1).expand_as(inputs)].view(-1, self.num_features)
        ...
        pid_logits = pid_inputs.mm(self.lut.t()) * self.oim_scalar
        if torch.isnan(loss_oim):
            loss_oim=torch.tensor([0.0]).to(inputs.device)
        return loss_oim, pid_inputs, pid_label, pid_logits

...
from models.teacher_head import Seq_Reid
from models.oim import OIMLoss
from models.resnet import build_resnet
from models.kd_loss import KLLoss, KD_feat_loss, KD_pkt_loss, KD_sim_kl_loss
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head, _ = build_resnet(name="resnet50", pretrained=True)
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        if 'kd_logits' in losses:
            losses['kd_logits'] *= self.lw_kdlogits
        if 'kd_feat' in losses:
            losses['kd_feat'] *= self.lw_kdfeat
        if 'oim_loss2' in losses:
            losses['oim_loss2'] *= self.lw_oim_loss2
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        self.teacher_head = Seq_Reid(cfg)
        self.kd_logits_loss = KLLoss(cfg.logits_temp, cfg.kl_bidir)
        self.kd_feat_loss = KD_feat_loss
        self.lw_kdlogits = cfg.lw_kdlogits
        self.lw_kdfeat = cfg.lw_kdfeat
        self.lw_oim_loss2 = cfg.lw_oim_loss2
        self.lw_kd_pkt = cfg.lw_kd_pkt
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        teacher_input = box_features.clone().detach()
        ...
        if self.training:
            ...
            loss_box_reid, feat_det, label_det, logits_det = self.reid_loss(box_embeddings, box_pid_labels)
            if self.lw_oim_loss2 < 1e-5:
                with torch.no_grad():
                    oim_loss2, feat_ins, label_ins, logits_ins = self.teacher_head(teacher_input, box_pid_labels)
            else:
                oim_loss2, feat_ins, label_ins, logits_ins = self.teacher_head(teacher_input, box_pid_labels)
                losses.update(oim_loss2=oim_loss2)

            if self.lw_kdfeat > 0:
                kd_feat = self.kd_feat_loss(feat_ins, feat_det)
                losses.update(kd_feat=kd_feat)
            if self.lw_kd_sim_kl > 0:
                kd_sim_kl = self.kd_sim_kl(feat_ins, feat_det, self.tempsimkl, self.simkl_bidir)
                losses.update(kd_sim_kl=kd_sim_kl * self.lw_kd_sim_kl)
            if self.lw_kd_pkt > 0:
                kd_pkt = self.kd_pkt(feat_ins, feat_det)
                losses.update(kd_pkt=kd_pkt * self.lw_kd_pkt)
            if self.lw_kdlogits > 0:
                kd_logits = self.kd_logits_loss(logits_det, logits_ins)
                losses.update(kd_logits=kd_logits)

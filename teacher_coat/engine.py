...
def train_one_epoch(cfg, model, optimizer, loaders, device, epoch, tfboard, softmax_criterion_s3):
    model.train()
    metric_logger = MetricLogger(delimiter="  ")
    metric_logger.add_meter("lr", SmoothedValue(window_size=1, fmt="{value:.6f}"))
    header = "Epoch: [{}]".format(epoch)

    data_loader = loaders.train_iter
    ...
    for i, (images, targets, camid) in enumerate(
        ...
        loss_dict, feats_reid_3rd, targets_reid_3rd = model(images, targets)
...
def evaluate_performance(
    ...
    if use_cache:
        ...
        for images, targets, camids in tqdm(gallery_loader, ncols=0):
            images, targets = images.to(torch.device('cuda')), targets.to(torch.device('cuda'))
            outputs = model(images)
            for output in outputs:
                gallery_feats.append(output.cpu().numpy().squeeze())

        query_feats = []
        for images, targets, camids in tqdm(query_loader, ncols=0):
            images, targets = to_device(images, targets, device)
            # targets will be modified in the model, so deepcopy it
            outputs = model(images)
            for output in outputs:
                query_feats.append(output.cpu().numpy().squeeze())
    ...
    return gallery_feats, query_feats

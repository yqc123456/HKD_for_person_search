...
class COAT_REID(nn.Module):
    ...
    def inference(self, images):
        ...
        features = self.roi_heads.pool(features['feat_res4'])
        box_features_3rd = self.roi_heads.box_head_3rd(features)
        embeddings, _ = self.roi_heads.embedding_head_3rd(box_features_3rd)
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        features = self.backbone(images)
        _, losses, feats_reid_3rd,targets_reid_3rd = self.roi_heads(features, targets)

        # apply loss weights
        losses["loss_rcnn_reid_3rd"] *= self.lw_rcnn_reid_3rd

        return losses, feats_reid_3rd, targets_reid_3rd
...
class CascadedROIHeads(nn.Module):
    ...
    def forward(self, features, targets=None):
        ...
        # --------------------- The third stage -------------------- #
        box_features = self.pool(features['feat_res4'])
        box_features = self.box_head_3rd(box_features)
        box_embeddings_3rd, box_cls_scores_3rd = self.embedding_head_3rd(box_features)

        result, losses = [], {}
        loss_rcnn_reid_3rd, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(box_embeddings_3rd, targets)
        if self.training:
            losses.update(loss_rcnn_reid_3rd=loss_rcnn_reid_3rd)
        else:
            num_images = len(targets)
            result = [dict(labels=targets[i], embeddings=box_embeddings_3rd[i]) for i in range(num_images)]
        return result, losses, feats_reid_3rd,targets_reid_3rd

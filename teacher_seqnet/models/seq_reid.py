...
class Seq_Reid(nn.Module):
    ...
    def inference(self, images):
        ...
        box_features = self.roi_heads.reid_head(features)
    ...
    def forward(self, images, targets=None):
        ...
        features = self.backbone(images)
        _, losses = self.roi_heads(features, targets)
...
class SeqRoIHeads(nn.Module):
    ...
    def forward(self, features, targets=None):
        ...
        result, losses = [], {}
        if self.training:
            loss_box_reid = self.reid_loss(box_embeddings, targets)
            losses.update(loss_box_reid=loss_box_reid)
        else:
            num_images = len(targets)
            result = [dict(labels=targets[i], embeddings=box_embeddings[i]) for i in range(num_images)]
